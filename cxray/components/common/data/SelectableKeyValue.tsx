export default function SelectableKeyValue({name, value, selected, onClick, width, height, fontSize, keyRatio}: 
  ...
  return (
    ...
    style={{
      ...
      color: selected ? 'rgb(112,173,71)' : '#BBB',
      display: 'flex',
      alignItems: 'center'
    }} onClick={onClick}>
      <div style={{width: `${keyRatio ? keyRatio : 50}%`}}>
        <div style={{
          ...
          borderRight: `1px solid ${selected ? 'rgb(112,173,71)' : '#BBB'}`
        }}>
          {name}
        </div>
      </div>
